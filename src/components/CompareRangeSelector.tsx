...
export const CompareRangeSelector: React.FC<CompareRangeSelectorProps> = ({
  ...
  // Validation
  const isIdentical = Boolean(baseSha && headSha && baseSha.trim() === headSha.trim());
  const canCompare = Boolean(baseSha.trim() && headSha.trim() && !isIdentical && !isLoading);

  // Timeline ordering detection (index 0 is newest commit)
  const baseIndex = commits.findIndex((c) => c.sha.startsWith(baseSha) || baseSha.startsWith(c.sha));
  const headIndex = commits.findIndex((c) => c.sha.startsWith(headSha) || headSha.startsWith(c.sha));
  ...
  return (
    <div id="compare-range-selector" className="p-4 bg-zinc-900/60 border-b border-zinc-800 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={isLoading}
            className="px-3 py-1.5 bg-zinc-900 hover:bg-zinc-800 text-indigo-300 hover:text-indigo-200 text-xs font-mono rounded-lg border border-indigo-900/80 hover:border-indigo-700 flex items-center gap-1.5 transition-all shadow-sm"
            title={
              lastReviewedSha
          ...
          >
            ...
            {lastReviewedSha && (
              <span className="text-[10px] text-zinc-400 bg-zinc-950 px-1.5 py-0.2 rounded border border-zinc-800 font-mono">
      ...
      {noReviewNotice && (
        <div className="p-2.5 bg-amber-950/40 border border-amber-800/60 rounded-lg text-xs text-amber-300 flex items-center gap-2 font-mono animate-in fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
        ...
        <div className="md:col-span-5 space-y-1">
          ...
          {customBase ? (
            <input
              ...
              className="w-full px-3 py-2 bg-zinc-950 border border-zinc-700/80 rounded-lg text-xs font-mono text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:ring-1 focus:ring-indigo-500"
            />
          ) : (
            <select
              ...
              className="w-full px-3 py-2 bg-zinc-950 border border-zinc-700/80 rounded-lg text-xs font-mono text-zinc-100 focus:outline-none focus:ring-1 focus:ring-indigo-500 truncate"
            >
              ...
              {commits.map((c) => {
                ...
                return (
                  <option key={c.sha} value={c.sha} className="bg-zinc-900 text-zinc-200">
        ...
        <div className="md:col-span-2 flex justify-center">
          <button
            ...
            type="button"
            onClick={handleSwap}
            className="p-2 bg-zinc-800/80 hover:bg-zinc-700 text-zinc-300 rounded-lg border border-zinc-700/80 transition-colors"
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
        ...
        <button
          ...
          disabled={!canCompare}
          className="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white rounded-lg text-xs font-bold font-mono flex items-center gap-2 shadow-md shadow-indigo-600/20 transition-all ml-auto"
        >
          {isLoading ? (
            <>
      ...
      {compareResult && (
        <div
          ...
          className="p-3 bg-zinc-950 border border-indigo-900/60 rounded-xl space-y-2 text-xs font-mono animate-in fade-in"
        >
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            {compareResult.html_url && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-[11px] text-zinc-400 hover:text-indigo-300 flex items-center gap-1"
              >
          ...
          <div className="flex flex-wrap items-center gap-3 text-[11px] text-zinc-400">
            <span>
              Base: <strong className="text-zinc-200 font-mono">{baseSha.substring(0, 7)}</strong>
            </span>
            <span>
              Head: <strong className="text-zinc-200 font-mono">{headSha.substring(0, 7)}</strong>
            </span>
            <span>
              Files: <strong className="text-zinc-200">{compareResult.files?.length || 0}</strong>
            </span>
          </div>
