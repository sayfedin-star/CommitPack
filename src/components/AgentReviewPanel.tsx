...
export const AgentReviewPanel: React.FC<AgentReviewPanelProps> = ({
  ...
  return (
    <div id="agent-review-panel" className="p-4 bg-white dark:bg-zinc-900/40 border border-slate-200 dark:border-zinc-800 rounded-xl space-y-3.5">
      {/* Panel Header */}
            ...
            <button
              ...
              className={`px-2.5 py-1 text-xs font-mono rounded-lg border flex items-center gap-1.5 transition-all ${
                isCurrentHeadReviewed || justMarked
                  ...
                  : 'bg-slate-100 hover:bg-slate-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-slate-700 dark:text-zinc-300 border-slate-300 dark:border-zinc-700'
              }`}
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-xs">
          <label htmlFor="review-task-textarea" className="font-semibold text-slate-700 dark:text-zinc-300 flex items-center gap-1">
          ...
          <div className="flex items-center gap-3 text-[11px] font-mono text-slate-500 dark:text-zinc-400">
            ...
            {hasTask && (
              <button
                ...
                className="text-slate-400 hover:text-rose-600 dark:text-zinc-500 dark:hover:text-rose-400 flex items-center gap-0.5"
                title="Clear task"
              >
        ...
        <textarea
          ...
          rows={4}
          className="w-full p-3 bg-white dark:bg-zinc-950 border border-slate-300 dark:border-zinc-700/80 rounded-lg text-xs font-mono text-slate-900 dark:text-zinc-100 placeholder:text-slate-400 dark:placeholder:text-zinc-600 focus:outline-none focus:ring-1 focus:ring-indigo-500 leading-relaxed shadow-xs"
        />
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 text-slate-700 dark:text-zinc-300 text-xs font-mono rounded-lg border border-slate-300 dark:border-zinc-700/80 flex items-center gap-1.5 transition-colors"
          >
            {isPreviewOpen ? (
              <>
                <EyeOff className="w-3.5 h-3.5 text-slate-400 dark:text-zinc-400" />
                <span>Hide Preview</span>
              </>
            ) : (
              <>
                <Eye className="w-3.5 h-3.5 text-slate-400 dark:text-zinc-400" />
          ...
          <button
            ...
            onClick={handleCopy}
            disabled={!hasTask}
            className={`px-4 py-1.5 rounded-lg text-xs font-bold font-mono flex items-center gap-1.5 shadow-sm transition-all ${
              hasTask
                ...
                : 'bg-slate-100 dark:bg-zinc-800 text-slate-400 dark:text-zinc-500 border border-slate-200 dark:border-zinc-700/60 cursor-not-allowed'
            }`}
          ...
          >
            {hasCopied ? (
              <>
                ...
                <span className="text-emerald-100">Copied Prompt!</span>
              </>
            ) : (
              <>
      ...
      {!hasTask && (
        <div className="p-2 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/40 rounded-lg text-[11px] text-amber-800 dark:text-amber-300 flex items-center gap-1.5 font-mono">
          <HelpCircle className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400 shrink-0" />
      ...
      {isPreviewOpen && (
        <div id="review-prompt-preview-box" className="p-3 bg-white dark:bg-zinc-950 border border-indigo-200 dark:border-indigo-900/60 rounded-xl space-y-2 animate-in fade-in shadow-xs">
          <div className="flex items-center justify-between text-xs font-mono text-slate-500 dark:text-zinc-400 pb-1 border-b border-slate-200 dark:border-zinc-800">
            <span className="font-semibold text-indigo-700 dark:text-indigo-300">Generated Review Prompt Preview</span>
            <span className="text-[11px]">Formatted Markdown</span>
          </div>
          <pre className="text-xs font-mono text-slate-800 dark:text-zinc-200 bg-slate-50 dark:bg-zinc-900/70 p-3 rounded-lg max-h-64 overflow-y-auto whitespace-pre-wrap leading-relaxed select-text border border-slate-200 dark:border-zinc-800">
            {fullPromptMarkdown}
          </pre>
        </div>
      )}
