...
export const TokenInput: React.FC<TokenInputProps> = ({
  ...
  const handleTestToken = async () => {
    ...
    try {
      const state = await checkRateLimit(inputValue.trim());
  ...
  return (
    ...
    >
      <div
        id="pat-modal-dialog"
        className="bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl max-w-lg w-full overflow-hidden animate-in fade-in zoom-in-95 duration-150"
      >
        {/* Modal Header */}
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="bg-zinc-950/70 border border-zinc-800/90 rounded-lg p-3.5 flex items-start gap-3">
            <Shield className="w-4 h-4 text-emerald-400 mt-0.5 shrink-0" />
            <div className="text-xs text-zinc-300 space-y-1">
              <p className="font-medium text-zinc-200">100% Client-Side & Secure</p>
              <p className="text-zinc-400 leading-relaxed">
                Your token is stored exclusively in your browser's <code className="text-indigo-300 font-mono">localStorage</code>. It is never logged, never sent to any backend, and passed strictly to <code className="text-indigo-300 font-mono">api.github.com</code> in the <code className="text-zinc-300 font-mono">Authorization</code> header.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-zinc-950 border border-zinc-700 rounded-lg px-3 py-2 text-xs font-mono text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500"
              />
          ...
          <div className="text-[11px] text-zinc-400 space-y-1 bg-zinc-950/40 p-3 rounded border border-zinc-800/60">
            <span className="font-semibold text-zinc-300">Recommended Scopes:</span>
            <ul className="list-disc pl-4 space-y-0.5 text-zinc-400">
              <li>For public repos: No scopes required (or <code className="text-zinc-300">public_repo</code>)</li>
              <li>For private repos: <code className="text-zinc-300">repo</code> (Full control of private repositories)</li>
            </ul>
          </div>
        </div>
        ...
        <div className="px-5 py-3.5 bg-zinc-950/60 border-t border-zinc-800 flex items-center justify-between">
          <div>
            {token && (
              <button
                ...
                type="button"
                onClick={handleClear}
                className="text-xs text-rose-400 hover:text-rose-300 flex items-center gap-1.5 px-2 py-1 rounded hover:bg-rose-950/30 transition-colors"
              >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 rounded-lg text-xs font-medium text-zinc-300 bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 disabled:opacity-50 transition-colors"
            >
            ...
            <button
              ...
              type="button"
              onClick={handleSave}
              className="px-4 py-1.5 rounded-lg text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-500 shadow-md shadow-indigo-600/20 transition-all"
            >
              Save & Apply
            </button>
          </div>
